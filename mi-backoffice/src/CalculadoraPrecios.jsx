...
export default function CalculadoraPrecios() {
  ...
  const handleCSV=e=>{
    ...
    r.onload=async ev=>{
      const p=parseCatalogCSV(ev.target.result);
      if(p.length>0){
        setProductos(p);
        await DB.guardarCatalogo('productos', p);
        alert(`${p.length} productos cargados y guardados.`);
  ...
  const addProduct=async ()=>{
    ...
    const n=[...productos,prod];
    setProductos(n);
    await DB.guardarCatalogo('productos', n);
    setSelected(prod);
    setShowNew(false);
  ...
  const updateProducto=async ()=>{
    const c = parseFloat(tempProd.costo) || 0;
    const updated = { ...selected, ...tempProd, costo: c };
    setSelected(updated);
    const n = productos.map(p => p.codigo === selected.codigo ? updated : p);
    setProductos(n);
    await DB.guardarCatalogo('productos', n);
    setEditMode(false);
  };
              ...
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                ...
                <tbody>
                  {filtered.map(p=>{
                    ...
                    return(
                      <tr key={p.codigo} onClick={()=>{setSelected(p);setUnidades(1);setEditMode(false);}}
                        style={{borderBottom:`1px solid ${T.border}`,cursor:'pointer',                          background:sel?T.accentBg:'transparent',transition:'background 0.1s'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:'12px 14px',display:'flex',flexDirection:'column',gap:10}}>
              <div>
                ...
                {editMode?(
                  <div style={{display:'flex',flexDirection:'column',gap:10,marginTop:6,background:T.surface,padding:12,borderRadius:8,border:`1px solid ${T.border}`}}>
                    <div style={{color:T.cyan,fontSize:10,fontWeight:700,letterSpacing:'0.1em',fontFamily:"'DM Sans',sans-serif",marginBottom:4}}>MODO EDICIÓN RÁPIDA</div>
                    <div style={{display:'flex',gap:10}}>
                      <SI label="NOMBRE DE VENTA" value={tempProd.nombre} onChange={v=>setTempProd({...tempProd,nombre:v})}/>
                    </div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
                      <SS label="LÍNEA" value={tempProd.linea} onChange={v=>setTempProd({...tempProd,linea:v})} options={LINS}/>
                      <SS label="MONEDA" value={tempProd.moneda} onChange={v=>setTempProd({...tempProd,moneda:v})} options={['USD','GS']}/>
                      <div style={{display:'flex',flexDirection:'column',gap:4}}>
                        <SL>COSTO BASE</SL>
                        <input type="number" value={tempProd.costo} onChange={e=>setTempProd({...tempProd,costo:e.target.value})}
                          style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:6,padding:'7px 10px',
                            color:T.textPrimary,fontSize:12,fontFamily:"'JetBrains Mono',monospace",outline:'none'}}/>
                      </div>
                    </div>
                    <div style={{display:'flex',gap:6,marginTop:4}}>
                      <button onClick={updateProducto}
                        style={{background:T.green,border:'none',borderRadius:6,padding:'6px 14px',color:'#000',
                          fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>Guardar Cambios</button>
                      <button onClick={()=>setEditMode(false)} 
                        style={{background:'transparent',border:`1px solid ${T.border}`,borderRadius:6,padding:'6px 14px',
                          color:T.textMuted,cursor:'pointer',fontSize:11,fontFamily:"'DM Sans',sans-serif"}}>Cancelar</button>
                    </div>
                  </div>
                ):(
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    ...
                    <button onClick={()=>{setTempProd({...selected});setEditMode(true);}}
                      style={{background:'transparent',border:`1px solid ${T.border}`,borderRadius:4,padding:'2px 6px',
                        color:T.textMuted,fontSize:9,cursor:'pointer',display:'flex',alignItems:'center',gap:2}}>
                      <Edit3 size={8}/> Editar
                    </button>
