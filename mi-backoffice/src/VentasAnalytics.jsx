...
const SL=({children})=><label style={{fontSize:10,fontWeight:700,color:T.textSecondary,fontFamily:"'DM Sans',sans-serif"}}>{children}</label>;
const SS=({label,value,onChange,options})=>(
  <div style={{display:'flex',flexDirection:'column',gap:4,flex:1,minWidth:0}}>
    ...
    <select value={value} onChange={e=>onChange(e.target.value)}
      ...
      padding:'6px 10px',fontSize:11,color:T.textPrimary,fontFamily:"'DM Sans',sans-serif",outline:'none',cursor:'pointer'}}>
...
const SI=({label,value,onChange,type='text',placeholder,readOnly,mono})=>(
  <div style={{display:'flex',flexDirection:'column',gap:4,flex:1,minWidth:0}}>
    ...
    <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} readOnly={readOnly}
      style={{width:'100%',background:readOnly?T.surface:T.bg,border:`1px solid ${T.border}`,borderRadius:8,
      padding:'6px 10px',fontSize:11,color:readOnly?T.textMuted:T.textPrimary,fontFamily:mono?"'JetBrains Mono',monospace":"'DM Sans',sans-serif",outline:'none'}}/>
  </div>
);
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div style={{borderBottom:`1px solid ${T.border}`,display:'flex',gap:0}}>
        <TabBtn id="tendencia" label="📈  Tendencia" tab={tab} setTab={setTab}/>
        <TabBtn id="estacionalidad" label="🌊  Estacionalidad" tab={tab} setTab={setTab}/>
        <TabBtn id="comparativa" label="⚖️  Comparativa" tab={tab} setTab={setTab}/>
        <TabBtn id="categorias" label="🏷️  Categorías" tab={tab} setTab={setTab}/>
        <TabBtn id="tintas" label="🎨  Tintas & Stock" tab={tab} setTab={setTab}/>
        <TabBtn id="compras" label="📦  Compras" tab={tab} setTab={setTab}/>
        <TabBtn id="stock" label="⚖️  Stock & Rotación" tab={tab} setTab={setTab}/>
      </div>
              ...
              <thead>
                <tr style={{background:T.surface}}>
                  ...
                  <th onClick={()=>handleSort('qty')} style={{padding:'10px 14px',color:sortField==='qty'?T.accent:T.textMuted,fontSize:10,fontWeight:700,letterSpacing:'0.08em',textAlign:'right',borderBottom:`1px solid ${T.border}`,fontFamily:"'DM Sans',sans-serif",cursor:'pointer',userSelect:'none'}}>
                    <span style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:4}}>CAJAS VENDIDAS <SortIcon field="qty" sortField={sortField} sortDir={sortDir}/></span>
                  </th>
                  <th onClick={()=>handleSort('txns')} style={{padding:'10px 14px',color:sortField==='txns'?T.accent:T.textMuted,fontSize:10,fontWeight:700,letterSpacing:'0.08em',textAlign:'right',borderBottom:`1px solid ${T.border}`,fontFamily:"'DM Sans',sans-serif",cursor:'pointer',userSelect:'none'}}>
                    <span style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:4}}>NRO. VENTAS <SortIcon field="txns" sortField={sortField} sortDir={sortDir}/></span>
                  </th>
